...
public:
  ...
  // recovery blocks standing in for the ones that were not, in the order they
  // are submitted after the blocks that were found. An implementation that
  // solves the erasure for itself returns true, and is then given the index of
  // each input block instead of a column of coefficients; the caller does not
  // invert the matrix at all. Only repair can offer this, creation having no
  // erasures to solve.
  virtual bool SetErasures(const std::vector<bool> &present, const u16 *exponents, u32 count)
  {
    (void)present;
    (void)exponents;
    (void)count;
  ...
  // SetRecoveryExponents or SetErasures returned true. The returned future
  // becomes ready once data may be overwritten.
