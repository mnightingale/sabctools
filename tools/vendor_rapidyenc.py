...
# that wraps both. Upstream's CMakeLists.txt is what setup.py drives.
#
...
def apply_patches():
    ...
    Unlike par2cmdline-turbo, upstream leaves the MSVC runtime library alone, so the
    CMAKE_MSVC_RUNTIME_LIBRARY default of /MD already suits a CPython extension.
    """
...
[crcutil](https://code.google.com/p/crcutil/) and zlib-ng's folding approach. It supersedes the
two trees sabctools used to carry by hand, `src/yencode/` and `src/crcutil-1.0/`.
...
`setup.py` drives upstream's own CMake build to produce the `rapidyenc` static library and
links it into the extension, the same way it builds `src/par2/`. Upstream owns the per-ISA
flag matrix - which files get `-mavx2`, which get `-march=armv8-a+crc`, and which ISA probes
have to pass first - so re-vendoring picks up new kernels without any change here.
...
None. Each vendoring script fails loudly if a patch it carries stops matching, so this
